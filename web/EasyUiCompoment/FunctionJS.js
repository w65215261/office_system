...
/* top:       左上位置 %（看文档定义）
/*closeFunc   回调函数
/*retStr      返回值 必须是个对象，否则，只是形参的传入
/*btnClose      true or false  是否显示 关闭按钮
---------------------------------------------------*/
//function openDialogWithCallback(url, _id, _title, _width, _height, left, top, closeFunc, btnClose) {
function openDialogWithCallback(url, _id, _title, _width, _height, closeFunc, btnClose) {
    art.dialog.open(url, {
        id: _id,
        title: _title,
        width: _width,
        ...
        lock: true,
        resize: false,
        cancel: (btnClose == true ? undefined : btnClose),               // 是否显示“关闭”按钮
        esc: false, 				// 是否支持Esc键关闭
        allowDBLClick: false,       //是否允许双击关闭
        //cancel函数，点击右上角X的执行函数，返回false 禁止关闭，三个事件： ESC、X按钮、双击关闭，会触发该函数
        //cancel: function () { return false;},   
        init: function () { art.dialog.data('RetVal', ''); },                  //首先将返回值，清空
        close: function () {                                                  //获取返回值，回调函数

            if (closeFunc != null)
            // alert(window.frames[this.config.id].returnVal);
            //  closeFunc(art.dialog.data('RetVal'));
            // closeFunc(window.frames[_id].returnVal);
                closeFunc(window.frames[this.config.id].returnVal);         //结果是一样的
//            	closeFunc('true');
        }
...
/*parm：      条件参数
--------------------------------------------------*/
function ResetPwdConfig(url, parm, count, callbackFun) {
    ResetPasswordConfig(url, parm, "注：重置密码为：1234，操作不可恢复，您确定要继续么？", count, callbackFun);
}
function ResetPasswordConfig(url, parm, Msg, count, callbackFun) {
    if (count == undefined) {
        count = 1;
    }
    $.messager.confirm('警告', Msg, function (r) {
        if (r) {
            ...
            getAjax(url, parm, function (rs) {
                ...
                if (data.success) {
                    showTipsMsg("成功重置密码 " + count + " 笔记录。", 2000, 4);
                    //回调页面函数
                    callbackFun();
...
/*parm：      条件参数
--------------------------------------------------*/
function MoveConfig(url, parm, callbackFun) {
    getAjax(url, parm, function (rs) {
        var data = rs;
        //返回对象类型转换为json
        if ((typeof (data)).toLocaleLowerCase() == "string") {
        ...
        if (data.success) {
            showTipsMsg(data.message, 2000, 4);
            //回调页面函数
            callbackFun();
        } else if (data.success == false) {
            showTipsMsg("排序失败，请稍后重试", 4000, 5);
        } else {
            showTipsMsg(rs, 4000, 3);
        }
    });
...
/*拖拉移动节点*/
function drop(url, parm, callbackFun) {
...
/*parm：      条件参数
--------------------------------------------------*/
function delConfig(url, parm, count, callbackFun) {
    DeleteConfig(url, parm, "注：删除操作不可恢复，您确定要继续么？", count, callbackFun);
}
function DeleteConfig(url, parm, Msg, count, callbackFun) {
    if (count == undefined) {
        count = 1;
    }
        ...
        if (r) {
            ...
            getAjax(url, parm, function (rs) {
                ...
                if (data.success == true) {
                    if (data.contaionchid == true) {
                        showTipsMsg("节点有子节点，不能删除", 4000, 3);
                    }
                    else if (data.Contation == true) {
                        showTipsMsg(data.message, 2000, 4);
                        //回调页面函数
                        callbackFun();
                    }
...
function setDefaultConfig(url, parm, count, callbackFun) {
    if (count == undefined) {
        count = 1;
    }
    //alert('confirmed:' + r);
    getAjax(url, parm, function (rs) {
        ...
        if (data.success == true) {
            showTipsMsg(data.message, 4000, 4);
            callbackFun();
        } else if (data.success == false) {
            showTipsMsg(data.message, 4000, 5);
        }
    });
}
/*验证数据是否存在
...
function getAjaxfortext(url, parm, callBack) {
    $.ajax({
        type: 'post',
        dataType: "text",
        url: url,
...
/*tab页自适应浏览器高度 create by sc 2014.04.29*/
function windowResize() {
    ...
    $('.box').find('.boxone').width((boxWidth) / 2 - 20);
}
/*tab页自适应浏览器高度 create by sc 2014.04.29*/
...
//获取页面操作按钮guid编号 Create by sc 2014.5.7
//MethodName:方法名称,例如Add\Edit\Look   ;   type:按钮类型,例如toolbar(工具栏按钮)\operation(行操作按钮)
function GetButtonGuidByMethodName(MethodName, type) {
    //alert($("#Hf_TopButton").val());
    var btnInfos = "";
    if (type == "toolbar") {
        btnInfos = $("#Hf_ToolBarButton").val().split(',');
    }
    else if (type == "operation" && $("#Hf_OperationButton").val() != "") {
        btnInfos = $('#Hf_OperationButton').val().split('^')[1].split(',');
    }
    for (var i = 0; i < btnInfos.length; i++) {
        var btninfo = btnInfos[i].split('|');
        if (btninfo[0] == MethodName) {
            return btninfo[1];
        }
    }
...
//电大 datagrid的封装工具类 Create by sc 2014.5.8
var DDgrid = {
    Config: {
        autoLoad: true,   //默认自动加载数据
        gridID: '',       //datagrid的id，全字符，需要加#
        operaStr: ''      //grid中行后面操作项的字符串
    }
};
DDgrid.Util = {

    //初始化datagrid，参数对象id，工具栏ID，url，显示列，冻结列，主键,操作项中的字符串
    InitDataGrid: function (toolObj1, url1, columns1, frColumns1, idFiled1) {
        $('#' + this.GetDgID()).datagrid({
            singleSelect: false,
            collapsible: true,
            remoteSort: false,
            multiSort: true,
            ...
            queryParams: {},      //设置查询条件参数，从条件框中得到
            idField: idFiled1,
            url: url1 + '&rnd=' + NewGuid(),
            toolbar: "#" + toolObj1, //工具栏divID
            columns: columns1,
            frozenColumns: frColumns1, //冻结列
            //--------------------一下为事件处理，放置最后
            ...
            onBeforeLoad: function () {
                ...
                return DDgrid.Config.autoLoad;     //返回false，将不会加载数据
            },
            onHeaderContextMenu: function (e, field) {
                //e.preventDefault();
                //alert(field);
            },
            //点击列header，进行排序，根据查询条件和排序字段，进行查询QueryParam  SortField SortType
            //此函数这里不能用this.GetDgID()，这里的this代表当前datagrid而非DDgrid对象
            onSortColumn: function (sort, order) {
                DDgrid.Util.SearchByFieldsCBEncap(sort + "|" + order);
            }
    ...
    //设置行操作字符串  operaStr
    SetOperaStr: function (pStr) {
        DDgrid.Config.operaStr = pStr;
    },
    //设置datagrid的ID
    SetDgID: function (pid) {
        DDgrid.Config.gridID = pid;
    },
    //得到行操作字符串  operaStr
    GetOperaStr: function () {
        return DDgrid.Config.operaStr;
    },
    //得到datagrid的ID
    GetDgID: function () {
        return DDgrid.Config.gridID;
    },
    //编辑功能封装
    EditEncap: function (id1, methodName1, idFiled1, url1, title1, width1, height1, closeFunc1, btnClose1) {
        var ids = "";
        var btnGuid = "";
        if (id1 != undefined) {
            btnGuid = GetButtonGuidByMethodName(methodName1, "operation"); //获取点击按钮的id
            ids = id1;
        }
        else {
            btnGuid = GetButtonGuidByMethodName(methodName1, "toolbar"); //获取点击按钮的id

            //获取datagird选中行数据(多行)
            var rowDatas = $('#' + this.GetDgID()).datagrid('getSelections');

            //循环得到每行的某个字段值，例如：PK_CTDM_GUID
            rowDatas.forEach(function (e) {
                ids += e[idFiled1] + ",";
            });
            if (ids.length > 0) {
        ...
        //datagrid只能选中一行判断提示
        if (IsEditdata(ids)) {
            //调用封装的模态对话框(见functionJS.js)
            var url = url1 + "&btnId=" + btnGuid + "&id=" + ids + "&rnd=" + NewGuid();
            top.openDialogWithCallback(url, NewGuid(), title1, width1, height1, closeFunc1, btnClose1); //使用回调函数
        }
    },
    //编辑功能封装
    SpaceMenuEncap: function (id1, methodName1, idFiled1, url1, title1, width1, height1, closeFunc1, btnClose1) {
        var ids = "";
        var btnGuid = "";
        if (id1 != undefined) {
            btnGuid = GetButtonGuidByMethodName(methodName1, "operation"); //获取点击按钮的id
    ...
    //上移功能封装  MoveType('up'或者'down'分别表示上移或下移)
    MoveEncap: function (id1, methodName1, idFiled1, url1, MoveType, Func1) {
        var ids = "";
        var btnGuid = "";
        if (id1 != undefined) {
            btnGuid = GetButtonGuidByMethodName(methodName1, "operation"); //获取点击按钮的id
        ...
        if (IsEditdata(ids)) {
            //调用封装的模态对话框(见functionJS.js)
            var Moveparm = { 'CMD': 'Move', 'ids': ids, 'MoveType': MoveType, 'rnd': NewGuid() };
            MoveConfig(url1, Moveparm, Func1);
        }
    },
    //分配角色功能封装
    AllotRoleEncap: function (id1, methodName1, idFiled1, url1, title1, width1, height1, closeFunc1, btnClose1) {
        var ids = "";
        var btnGuid = "";
        if (id1 != undefined) {
            btnGuid = GetButtonGuidByMethodName(methodName1, "operation"); //获取点击按钮的id
    ...
    //查看功能封装
    LookEncap: function (id1, methodName1, idFiled1, url1, title1, width1, height1, closeFunc1, btnClose1) {
        var ids = "";
        var btnGuid = "";
        if (id1 != nudefined) {
            btnGuid = GetButtonGuidByMethodName(methodName1, "operation"); //获取点击按钮的id
    ...
    //删除功能封装
    DeleteEncap: function (id1, methodName1, idFiled1, url1, Func1) {
        var ids = "";
        var btnGuid = "";
        var count = 0; //删除的记录数
        if (id1 != undefined) {
        ...
        if (IsChecked(ids)) {
            //调用封装的模态对话框(见functionJS.js)
            var delparm = { 'CMD': 'Delete', 'ids': ids, 'rnd': NewGuid() };
            delConfig(url1, delparm, count, Func1);
        }
    },
    //设为默认功能封装
    SetDefaultEncap: function (id1, methodName1, idFiled1, url1, Func1) {
        var ids = "";
        var btnGuid = "";
        var count = 0; //删除的记录数
        if (id1 != undefined) {
        ...
        if (IsEditdata(ids)) {
            //调用封装的模态对话框(见functionJS.js)
            var delparm = { 'CMD': 'SetDefault', 'ids': ids, 'rnd': NewGuid() };
            setDefaultConfig(url1, delparm, count, Func1);
        }
    },
    //用户管理重置密码的封装
    ResetPasswordEncap: function (id1, methodName1, idFiled1, url1, Func1) {
        var ids = "";
        var btnGuid = "";
        var count = 0; //重置密码的记录数
        if (id1 != undefined) {
            btnGuid = GetButtonGuidByMethodName(methodName1, "operation"); //获取点击按钮的id
        ...
        if (IsChecked(ids)) {
            //调用封装的模态对话框(见functionJS.js)
            var resetparm = { 'CMD': 'ResetPassword', 'ids': ids, 'rnd': NewGuid() };
            ResetPwdConfig(url1, resetparm, count, Func1);
        }
    },
    //多字段排序查询
    SearchByFieldsEncap: function () {
        var columns = $('#' + this.GetDgID()).datagrid('options').columns[0]; //获取页面datagrid列名称集合

        var myReg = /^[\u4e00-\u9fa5]+$/; //判断中文正则表达式
        var columsJson = "";
        columns.forEach(function (e) {
            if (myReg.test(e.title) && e.title != '操作') {
                columsJson += "{'title':'" + e.title + "','value':'" + e.field + "'},";
            }
        });
        if (columsJson.length > 0) {
            columsJson = columsJson.substring(0, columsJson.length - 1);
        }
        columsJson = "[" + columsJson + "]";
        var url = "SetSorts.aspx?&SortFields=" + columsJson + "&rnd=" + NewGuid();
        //调用封装的模态对话框(见functionJS.js)
        top.openDialogWithCallback(url, NewGuid(), '自定义排序', 410, 275, this.SearchByFieldsCBEncap, true); //使用回调函数
    },
    //通用查询功能
    SearchEncap: function () {
        $('#' + this.GetDgID()).datagrid('load', {
            QueryParam: this.SearchConditionCallBack()
        });
        $('#' + this.GetDgID()).datagrid('clearSelections');
        $('#' + this.GetDgID()).datagrid('clearChecked');
    },
    //**********回调函数，供按多字段排序搜索使用，可以传入参数 sortinfo格式例如:"字段1,字段2,字段3|asc(desc)"
    //此函数不能用this.GetDgID()，在多字段排序中，回调次函数，this代表当前window而非DDgrid对象
    SearchByFieldsCBEncap: function (sortinfo) {
        var sort = "";
        var order = "asc";
        if (sortinfo != "" && sortinfo != undefined) {
            sort = sortinfo.split('|')[0];
            order = sortinfo.split('|')[1];
        }
        $('#' + DDgrid.Util.GetDgID()).datagrid('load', {
            QueryParam: DDgrid.Util.SearchConditionCallBack(),
            SortField: sort,
            SortType: order
        });
        $('#' + DDgrid.Util.GetDgID()).datagrid('clearSelections');
        $('#' + DDgrid.Util.GetDgID()).datagrid('clearChecked');
    },
    //查询条件的回调
    SearchConditionCallBack: function () {
...
//电大treegrid封装
var DDtreeGrid = $.extend(true, DDgrid, {   //扩充工具的方法
    Util: {
        InitTreeDataGrid: function (toolObj1, url, treeField1, columns1, frColumns1, idFiled1) {
            $('#' + this.GetDgID()).treegrid({
                nowrap: false,
                rownumbers: true,
                animate: true,
                collapsible: true,
        ...
        DropEncap: function (url, parm, callbackFun) {
            //调用封装的模态对话框(见functionJS.js)
            drop(url, parm, callbackFun);
        },
        //通用查询功能
        SearchEncap: function () {
            $('#' + this.GetDgID()).datagrid('load', {
        ...
        AddTreeGridEncap: function (url, parm, callbackFun) {
            getAjax(url, parm, function (data) {
                if (data.success) {
                    $('#' + DDtreeGrid.Util.GetDgID()).treegrid('append', {
                        parent: data.record_id,
                        data: [{
                            id: data.id,
                            name: data.title
                        }]
 ...
 * 
 */
function openJsp(paraType,paraId){
	parent.art.dialog.open('../../page/imageView/imageView.jsp?paraType='+paraType+'&paraId='+paraId,{
		id: '11',
        title: '照片',
        width: '645px',
        height: '450px',
...
function setDate(){
	...
	var day = now.getDate();
	var now_day = year + "-" + month + "-" + day;
	//判断表单的时间，如果时间为空，则给当天的时间
	if($("#factorydate").datebox('getValue')==""){
		$("#factorydate").datebox('setValue',now_day);
	}
	if($("#rundate").datebox('getValue')==""){
		$("#rundate").datebox('setValue',now_day);
	}
	if($("#registrationdate").datebox('getValue')==""){
		$("#registrationdate").datebox('setValue',now_day);
	}
}
