...
<style>
  ...
  #responsiblePersonOid3{
    cursor: pointer;
  }
  #receiverLabel3, #popoverSpan{
    cursor: pointer;
  }
</style>
<!-- 日期DIV-->
    ...
    <tr>
      <td>
        ...
        <div style="width: auto;float: left; "id="divColor">
          <div style="border-right:dashed #808080 1px;width: 40px;text-align: center;float: left;" >
            <c:if test="${timeType=='1'}">
              <span class="week"  id="0" onclick="chooseMonth()" style="color: #ff0000">月度</span>
            </c:if>
            <c:if test="${timeType!='1'}">
              <span class="week"  id="0" onclick="chooseMonth()">月度</span>
            </c:if>
          </div>

          <c:forEach begin="1" end="${mCount}" varStatus="a">
            <c:if test="${a.count!=currWeek}">
              <div style="border-right:dashed #808080 1px;width: 60px;text-align: center;float: left" ><span class="week" id="${a.count}" onclick="chooseWeek(${a.count})">第${a.count}周</span></div>
            </c:if>
            <c:if test="${a.count==currWeek}">
              <div style="border-right:dashed #808080 1px;width: 60px;text-align: center;float: left;" ><span class="week"  id="${a.count}" onclick="chooseWeek(${a.count})" style="color: #ff0000">第${a.count}周</span></div>
            </c:if>
          </c:forEach>
          <input type="hidden" id="chooseYear" value="${year}"><input id="month" type="hidden" value="${month}"><input id="cWeek" type="hidden" value="${currWeek}">
          <input id="timeType" type="hidden" value="0">
          <input id="showFlag" type="hidden" value="show1">
        </div>
      </td>
    </tr>
  </table>
        ...
        <div class="row">
          <section class="col col-6">
            <label class="input"> <i class="icon-append fa fa-calendar"></i>
              <input type="text" name="startTime" id="startTime2" readonly="true" onchange="updStartTime()" placeholder="选择开始日期">
            </label>
          </section>
          <section class="col col-6">
            <label class="input"> <i class="icon-append fa fa-calendar"></i>
          ...
          <form class="smart-form" id="createProject3">
            <fieldset>
              <section>
                ...
                <label class="input">
                  <input type="text" class="input-lg" name="taskName" id="taskName3" onblur="checkTaskName3()">
                </label>
              </section>
              <section>
                <div class="row">
          ...
          <fieldset>
            <section>
              <label class="input">
                ...
                <input type="text" class="input-lg" name="workHourName" id="workHourName" placeholder="工时名称" onblur="checkWorkHourName()">
              </label>
            </section>
            <section>
              <div class="row">
          ...
          <section>
            <p class="input-group">
              ...
              <input type="text" class="form-control" id="taskName" name="taskName"
                     placeholder="请输入计划名称" onblur="checkTaskName()"/>
            </p>
          </section>
          <!--折叠控件start-->
      ...
      success: function (data) {
        for(var i=0;i<data.length;i++){
          $("#Experiencelist").append(
                  ...
                  "<input type='text' name='endTime' id='endExperienceDate"+i+"' onchange='saveExperience(\""+data[i].oid+"\","+i+")'  class='endExperienceDate' style='border-width: 0px 0px;width: 100px' value='"+data[i].endTime+"'>"+
                  "</label>"+
...
//    loadURL("/plan/showPlan.do?year="+year+"&month="+month+"&week="+week,$('#s1'));
  ...
  function chooseWeek(week){
    ...
    console.log(week);
//    loadURL("/plan/showPlan.do?year="+year+"&month="+month+"&week="+week,$('#s1'));
    loadURL("/plan/showAddPlan.do?year="+year+"&month="+month+"&flag=1"+"&week="+week+"&chooseWeek=0"+"&planTimeType="+planTimeType, $('#inbox-content > .table-wrap'))
    $("#timeType").attr("value","0")
    $("#cWeek").attr("value",week)
  }
  <!-- 选择月-->
  function chooseMonth(){
    ...
    var month=$("#month").val();
    $("#showFlag").val("show0");
    $("#timeType").attr("value","1");
    loadURL("/plan/showAddPlanMonth.do?year="+year+"&month="+month+"&planTimeType=1", $('#inbox-content > .table-wrap'));
//    loadURL("/plan/showAddPlan.do?year="+year+"&month="+month+"&flag=0"+"&chooseWeek=0", $('#inbox-content > .table-wrap'));
    $("#cWeek").attr("value","");
