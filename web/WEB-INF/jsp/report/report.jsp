...
<div id="inbox-content" class="inbox-body no-content-padding" style="height: auto;width: auto">
  <div class="inbox-side-bar" >
    <ul class="inbox-menu-sm">
      <li>
        <h2 class="page-title txt-color-blueDark hidden-tablet"><i class="fa fa-fw fa-desktop"></i> 总结 &nbsp;</h2>
        <ul>
            <select id="year" style="margin-left: 88px;">
              <option value="${year-2}" onclick="refreshYearCalendar()">${year-2}</option>
              <option value="${year-1}" onclick="refreshYearCalendar()" >${year-1}</option>
              <option value="${year}" selected="true" onclick="refreshYearCalendar()">${year}</option>
              <option value="${year+1}" onclick="refreshYearCalendar()">${year+1}</option>
            </select>
            <li>
              <c:forEach items="${month}" var="m" varStatus="t">
              <a href="javascript:void(0)" id="${t.count}report" onclick=refreshCalendar("${m}")>${m}月</a>
              </c:forEach>
            </li>
        </ul>
      </li>
...
<script type="text/javascript">
  ...
  function refreshCalendar(month){
    ...
    if(flag==1){
      ...
      loadURL('/report/refreshCalendar.do?year='+year+'&month='+month+'&flag='+flag+'', $('#inbox-content > .table-wrap'));
    }
      for(var i=1;i<=12;i++){
        var s=i+'report'
        if(i==month){
